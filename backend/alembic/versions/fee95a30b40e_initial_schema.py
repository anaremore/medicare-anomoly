...
Revision ID: fee95a30b40e
Revises: 
Create Date: 2026-04-15 19:14:12.480143
...
def upgrade() -> None:
    ...
    op.create_index('uq_address_normalized', 'addresses', ['normalized_address', 'city', 'state', 'zip'], unique=True)
    ...
    sa.Column('mid_name', sa.Text(), nullable=True),
    sa.Column('bus_name', sa.Text(), nullable=True),
    sa.Column('general_type', sa.Text(), nullable=True),
    ...
    sa.Column('waiver_state', sa.String(length=2), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_exclusions_city_state', 'exclusions', ['city', 'state'], unique=False)
    op.create_index('idx_exclusions_npi', 'exclusions', ['npi'], unique=False)
    ...
    sa.Column('taxonomy_desc', sa.Text(), nullable=True),
    ...
    sa.Column('practice_address_1', sa.Text(), nullable=True),
    sa.Column('practice_address_2', sa.Text(), nullable=True),
    ...
    op.create_index('idx_providers_enum_date', 'providers', ['enumeration_date'], unique=False)
    op.create_index('idx_providers_taxonomy', 'providers', ['taxonomy_code'], unique=False)
    op.create_index('idx_providers_zip', 'providers', ['practice_zip'], unique=False)
    op.create_table('provider_addresses',
    sa.Column('provider_npi', sa.String(length=10), nullable=False),
    sa.Column('address_id', sa.Integer(), nullable=False),
    sa.Column('address_purpose', sa.String(length=20), nullable=False),
    sa.Column('first_seen', sa.Date(), nullable=True),
    sa.Column('last_seen', sa.Date(), nullable=True),
    sa.ForeignKeyConstraint(['address_id'], ['addresses.id'], ),
    sa.ForeignKeyConstraint(['provider_npi'], ['providers.npi'], ),
    sa.PrimaryKeyConstraint('provider_npi', 'address_id', 'address_purpose')
    )
    op.create_table('risk_scores',
    sa.Column('npi', sa.String(length=10), nullable=False),
    sa.Column('address_clustering_score', sa.Numeric(precision=5, scale=2), nullable=True),
    sa.Column('entity_profile_score', sa.Numeric(precision=5, scale=2), nullable=True),
    sa.Column('network_association_score', sa.Numeric(precision=5, scale=2), nullable=True),
    sa.Column('geographic_risk_score', sa.Numeric(precision=5, scale=2), nullable=True),
    ...
    sa.ForeignKeyConstraint(['npi'], ['providers.npi'], ),
    sa.PrimaryKeyConstraint('npi')
    )
    op.create_index('idx_risk_composite', 'risk_scores', ['composite_score'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index('idx_risk_composite', table_name='risk_scores')
    op.drop_table('risk_scores')
    op.drop_table('provider_addresses')
    op.drop_index('idx_providers_zip', table_name='providers')
    op.drop_index('idx_providers_taxonomy', table_name='providers')
    op.drop_index('idx_providers_enum_date', table_name='providers')
    op.drop_table('providers')
    op.drop_index('idx_exclusions_npi', table_name='exclusions')
    op.drop_index('idx_exclusions_city_state', table_name='exclusions')
    op.drop_table('exclusions')
    op.drop_index('idx_alerts_severity', table_name='alerts')
    op.drop_table('alerts')
    op.drop_index('uq_address_normalized', table_name='addresses')
